export * from './User/User.model';
export * from './User/UserFavoriteAnime.model';
export * from './User/UserFavoriteCharacter.model';

export * from './Anime/Anime.model';
export * from './Anime/AnimeHasCharacter.model';
export * from './Anime/AnimeHasTag.model';
export * from './Anime/AnimeTag.model';

export * from './Character/Character.model';
export * from './Character/CharacterHasTrait.model';
export * from './Character/CharacterTrait.model';
